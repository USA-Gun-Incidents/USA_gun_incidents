...
def clean_data_city_county(data):
    ...
    if check_parenthesis(data):
        ...
        if type(data1 is not str):
            data1 = str(data1)
        if type(data2) is not str:
            data2 = str(data2)
        return data1, data2
    else:
...
def check_consistency_geopy(row):
    ...
    if (row['state'] is not None and row['state_geopy'] is not None and type(row['state']) == str 
        and type(row['state_geopy']) == str): 
        state = clean_data_state(row['state']) # our data
        state_geopy = clean_data_state(row['state_geopy']) # GeoPy data
        # check state
        if state_geopy is not None and type(state_geopy) == str:
    ...
    if row['city_or_county'] is not None and type(row['city_or_county']) == str:
        city1, city2 = clean_data_city_county(row['city_or_county']) #our data
    ...
    return state_consistency, county_consistency, city_consistency, town_consistency
