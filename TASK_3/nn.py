...
clf_name = 'NeuralNetworkClassifier_Over'

# %%
# load the data
incidents_train_df = pd.read_csv('../data/clf_indicators_train_over.csv', index_col=0)
incidents_test_df = pd.read_csv('../data/clf_indicators_test.csv', index_col=0)
true_labels_train_df = pd.read_csv('../data/clf_y_train_over.csv', index_col=0)
...
param_grid = [
    {
        'nn__model__hidden_layer_sizes': [(256, 256,), (256, 256, 256,)],
        'nn__model__activation_functions': [('relu', 'relu','relu')],
        'nn__model__last_activation_function': ['sigmoid'],
        'nn__model__dropouts': [(0.1, 0.1,)],
        'nn__optimizer': ['adamax'],
        'nn__optimizer__learning_rate': [0.001, 0.002, 0.0005],
        'nn__optimizer__decay': [1e-5],
        'nn__loss': ['mean_squared_error'],
        'nn__batch_size': [256,512,1024],
        'nn__epochs': [10,20,30]
    },
...
gs = GridSearchCV( # RandomizedSearchCV?
    ...
    verbose=10,
    cv=StratifiedShuffleSplit(n_splits=3, test_size=1/3), # TODO: con altri classificatori usiamo stratified 5-fold, qui ci vuole troppo
    refit=True
)
...
params = {
    'nn__model__hidden_layer_sizes': [(256, 256, 256, ), (256, 256,), (128, 512, 128, )],
    'nn__model__dropout': [0, 0.5, 0.75],
    'nn__model__output_bias':[0, INITIAL_BIAS],
    'nn__model__final_activation_fun':['sigmoid', 'tanh'],
    'nn__batch_size': [256, 512, 1024, 2048],
    'nn__epochs': [250]
}
...
gs = GridSearchCV(pipe, params, scoring='accuracy', n_jobs=-1,
    verbose=True,
    cv=StratifiedShuffleSplit(n_splits=3, test_size=1/3), # TODO: con altri classificatori usiamo stratified 5-fold, qui ci vuole troppo
    refit=True)
